...
## Screen 4: Evidence Bag Popup
File: `Evidence Bag Popup.dc.html`

**Purpose:** In-scene popup, opened over the apartment/investigation view. Two modes, same component:
- **evidences mode:** player-opened bag/inventory. Header has a close "X" button + "EVIDENCES" title. Lists every evidence item collected so far (can run up to ~15+; list scrolls internally, max-height ~50vh/420px, no inner scrollbar visual chrome beyond default). Footer button bottom-right: "MAKE A MOVE".
- **next mode:** triggered by the game (e.g. after picking something up), no close button. Title "WHERE TO NEXT?". Shows only the most-recent item(s). Footer button: "KEEP EXPLORING".

**Layout:** Popup panel centered over a dimmed scene (scrim `rgba(14,17,20,0.35)` over the gameplay view), panel width ~min(92vw,760px), background `#e4e7ea`, border 6px `#0e1114`, padding 28px 32px 32px, flex column gap 22px.
- Header row: X close button (evidences mode only) — 40×40px, fill `#ccd2d8`, border 4px `#0e1114`, Press Start 2P 16px — + title, Press Start 2P 20px, ink `#242a30`.
- Item rows: no icons — flat text rows, fill `#f4f5f6`, border 4px `#0e1114`, padding 12px 18px, VT323 22px, ink `#242a30`, 10px gap between rows.
- Footer button: right-aligned, Press Start 2P 14px, fill `#4f8a86` (teal), ink `#e4e7ea`, border 4px `#0e1114`, padding 16px 28px, hover `#63a39d`.

**Data model:** a single `items: string[]` (evidence names, most-recent-last) + a `mode: 'evidences' | 'next'` flag from game state. In "next" mode, render only the single most-recently-added item (slice the list to its last entry), not a fixed index.

**Interactions:** X closes the popup with no side effect. "MAKE A MOVE" / "KEEP EXPLORING" close the popup and resume gameplay (exact downstream behavior — e.g. does "Make a Move" open a suspect-choice screen — is a gameplay hook outside this UI spec; ask if unclear).

**Assets:** none — no icons/photos in this popup, text rows only. Scrim background behind it is just gameplay dimmed, no placeholder art needed.
